...
(function () {
	...
	document.getElementById("contact_submit").addEventListener("submit", async event => {
		...
		.then(response => {
			...
			if(response.status===202){
				...
				document.querySelectorAll(".form_inputs").forEach(element => {
				...
				if(response.data.result==="error"){
					...
					for (const [field, message] of Object.entries(response.data.errors)) {
						innerHtml += `<p>${field}: ${message}</p>`
					}
					document.getElementById("email_errors").innerHTML = innerHtml
				}
